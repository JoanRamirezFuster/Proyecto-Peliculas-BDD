...
class PeliculaGenere
{
  ...
  public function inserir($servername, $username, $password, $genere, $titol)
  {
    ...
    try {
      $sql = "INSERT INTO Peli_Genere (Genere_id,Pelicula_id) VALUES ($genere, $titol)";
  ...
  public function consultaTots($servername, $username, $password)
  {
    ...
    try {
      $stmt = $conn->prepare("SELECT * FROM Peli_Genere");
      $result = $stmt->execute();
      $conn = null;
      return ($stmt);
    } catch (PDOException $e) {
  ...
  function modificar($servername, $username, $password, $id, $titol, $genere)
  {
    $conn = $this->connectar_bd($servername, $username, $password);
    try {

      $sql = "UPDATE Peli_Genere SET Titol='$titol'
    WHERE id='$id'";
